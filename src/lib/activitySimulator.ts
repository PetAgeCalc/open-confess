...
// Raw pool item shape before the contextual image is computed
type RawPostItem = Omit<SimulatedPostItem, 'rawImage'>;
...
// =========================================================================
// IMAGE MATCHING: every post gets a deterministic image whose subject tags
// match (a) the post's category (deep / funny / intelligent / incident /
// motivational / finance) and (b) the post's country, via LoremFlickr's
// keyword search + lock= (same post id -> same image forever, no repeats
// across the pool since each id is unique and feeds a unique lock number).
// =========================================================================

const CATEGORY_KEYWORDS: Record<SimulatedPostItem['category'], string> = {
  deep: 'portrait,thoughtful,person',
  funny: 'smile,candid,laughing',
  intelligent: 'office,meeting,discussion',
  incident: 'street,city,road',
  motivational: 'success,work,achievement',
  finance: 'business,office,city',
};

// Country -> keyword that biases LoremFlickr toward regionally-appropriate people/places.
// Add an entry here for every country used in CURATED_POST_POOL below.
...
// Builds a contextual, deterministic image URL for a post: same id -> same
// image every time, and the subject matter (people/place) matches the
// post's country + category instead of being a random abstract photo.
function buildContextualImageUrl(item: Pick<SimulatedPostItem, 'id' | 'category' | 'country'>): string {
  const catTag = CATEGORY_KEYWORDS[item.category] || 'people';
  const countryTag = COUNTRY_KEYWORDS[item.country] || '';
  const tags = [countryTag, catTag].filter(Boolean).join(',');
  const lock = hashStringToInt(item.id);
  return `https://loremflickr.com/700/500/${encodeURIComponent(tags)}?lock=${lock}`;
}

// =========================================================================
// 100 FULLY UNIQUE GLOBAL CONFESSIONS (English, Hindi, Bangla)
// Every single item now gets a 100% UNIQUE, deterministic, CONTEXTUALLY
// MATCHED image (country + category aware) computed below via
// buildContextualImageUrl, so nothing ever repeats across the entire
// 24-hour cycle AND the photo subject fits the story (e.g. an Indian post
// gets an Indian-tagged image, a funny post gets a lighter/candid image).
// Every author+story is fully unique (no repeated author/city/text
// combinations anywhere in the pool).
// =========================================================================
const RAW_POST_POOL: RawPostItem[] = [
  {
    id: 'en_deep_1',
    category: 'deep',
    city: 'New York',
...
// Final pool: each raw item + its contextually matched image (country + category aware, deterministic per id)
const CURATED_POST_POOL: SimulatedPostItem[] = RAW_POST_POOL.map((item) => ({
  ...item,
  rawImage: buildContextualImageUrl(item),
}));
