...
const USED_POST_IDS_KEY = 'openconfess_used_post_registry_v7';
const LIVE_SIMULATED_POSTS_KEY = 'openconfess_live_simulated_posts_v7';
const LAST_SIMULATION_TIMESTAMP_KEY = 'openconfess_last_drip_post_time_v7';
...
// - Other India: Mix of Hindi (2-3 posts) & English corporate/city reality
...
function updateSimulatedPostProgression(posts: Confession[]): Confession[] {
  ...
  return posts.map((post: any) => {
    ...
    const ageInMinutes = Math.max(0, Math.floor((now - Number(post.createdAt)) / (60 * 1000)));
    ...
    const visibleComments = poolItem.possibleComments.slice(0, targetCommentsCount).map((cText, idx) => {
      const commentTime = Number(post.createdAt) + (idx + 1) * (14 * 60 * 1000);
      return {
